...
// Package input contains functions related to the data input to be processed
package input
...
func Describe(input Input) (map[string]*FieldDescription, error) {
	input.Rewind()
...
func updateFieldNumBoundaries(value *dlit.Literal, fd *FieldDescription) {
	if fd.Kind == INT {
		...
		fd.MaxDP =
			int(maxI(int64(fd.MaxDP), int64(internal.NumDecPlaces(value.String()))))
	}
}
