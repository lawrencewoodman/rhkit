...
func TestNewAnd(t *testing.T) {
	cases := []struct {
		...
		{ruleA: NewGEFVF("rate", 1.05), ruleB: MustNewBetweenFVF("flow", 0.7, 2.1)},
		{ruleA: MustNewBetweenFVF("flow", 0.7, 2.1), ruleB: NewGEFVF("rate", 1.05)},
		{ruleA: MustNewOutsideFVI("flow", 1, 4), ruleB: NewGEFVI("flow", 0)},
		{ruleA: NewGEFVI("flow", 0), ruleB: MustNewOutsideFVI("flow", 1, 4)},
		{ruleA: MustNewOutsideFVF("flow", 0.7, 2.1), ruleB: NewGEFVF("flow", 0.2)},
		{ruleA: NewGEFVF("flow", 0.2), ruleB: MustNewOutsideFVF("flow", 0.7, 2.1)},
		{ruleA: MustNewOutsideFVI("rate", 0, 2),
			ruleB: NewGEFVI("rate", -5),
		},
		{ruleA: NewGEFVI("rate", -5),
			ruleB: MustNewOutsideFVI("rate", 0, 2),
		},
		{ruleA: MustNewOutsideFVF("flow", 0.7, 2.1),
			ruleB: NewGEFVF("flow", 0.5),
		},
		{ruleA: NewGEFVF("flow", 0.5),
			ruleB: MustNewOutsideFVF("flow", 0.7, 2.1),
		},
		{ruleA: MustNewOutsideFVI("rate", 0, 2),
			ruleB: NewLEFVI("rate", 4),
		},
		{ruleA: NewLEFVI("rate", 4),
			ruleB: MustNewOutsideFVI("rate", 0, 2),
		},
		{ruleA: MustNewOutsideFVF("flow", 0.7, 2.1),
			ruleB: NewLEFVF("flow", 5.5),
		},
		{ruleA: NewLEFVF("flow", 5.7),
			ruleB: MustNewOutsideFVF("flow", 0.7, 2.1),
		},
...
func TestNewAnd_errors(t *testing.T) {
	cases := []struct {
		...
		{ruleA: NewGEFVF("flow", 1.05),
			ruleB:      MustNewBetweenFVF("flow", 0.7, 2.1),
			wantErrStr: "can't And rule: flow >= 1.05, with: flow >= 0.7 && flow <= 2.1",
		},
		{ruleA: MustNewBetweenFVF("flow", 0.7, 2.1),
			ruleB:      NewGEFVF("flow", 1.05),
			wantErrStr: "can't And rule: flow >= 0.7 && flow <= 2.1, with: flow >= 1.05",
		},
		{ruleA: NewGEFVI("rate", 1),
			ruleB:      MustNewBetweenFVI("rate", 0, 2),
			wantErrStr: "can't And rule: rate >= 1, with: rate >= 0 && rate <= 2",
		},
		{ruleA: MustNewBetweenFVI("rate", 0, 2),
			ruleB:      NewGEFVI("rate", 1),
