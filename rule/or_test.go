...
func TestNewOr(t *testing.T) {
	cases := []struct {
		...
		{ruleA: NewGEFVF("flowA", 1.05),
			ruleB: MustNewOutsideFVF("flowB", 0.7, 2.1),
		},
		{ruleA: MustNewOutsideFVF("flowA", 0.7, 2.1),
			ruleB: NewGEFVF("flowB", 1.05),
		},
		{ruleA: NewGEFVI("rateA", 1),
			ruleB: MustNewOutsideFVI("rateB", 0, 2),
		},
		{ruleA: MustNewOutsideFVI("rateA", 0, 2),
			ruleB: NewGEFVI("rateB", 1),
		},
		{ruleA: MustNewOutsideFVI("rateA", 1, 5),
			ruleB: MustNewOutsideFVI("rateB", 7, 8),
		},
...
func TestNewOr_errors(t *testing.T) {
	cases := []struct {
		...
		{ruleA: NewGEFVI("rate", 1),
			...
			wantErrStr: "can't Or rule: rate >= 1, with: rate <= 0 || rate >= 2",
		},
		{ruleA: MustNewOutsideFVI("rate", 0, 2),
			ruleB:      NewGEFVI("rate", 1),
			wantErrStr: "can't Or rule: rate <= 0 || rate >= 2, with: rate >= 1",
		},
		{ruleA: MustNewOutsideFVI("rate", 1, 5),
			ruleB:      MustNewOutsideFVI("rate", 7, 8),
			wantErrStr: "can't Or rule: rate <= 1 || rate >= 5, with: rate <= 7 || rate >= 8",
		},
