...
func generateInFV(
	...
	for _, compareValues := range combinations(possibleLits, 2, maxNumLits) {
...
func getMaskValues(mask string, values []*dlit.Literal) []*dlit.Literal {
	r := []*dlit.Literal{}
	for j, b := range mask {
		if j >= len(values) {
			break
...
func combinations(values []*dlit.Literal, min, max int) [][]*dlit.Literal {
	r := [][]*dlit.Literal{}
	for i := 3; ; i++ {
		...
		if numOnBits >= min && numOnBits <= max && numOnBits <= len(values) {
			r = append(r, getMaskValues(mask, values))
		}
	}
	return r
}
