...
func TestAssessRulesMP(t *testing.T) {
	...
	t.Logf("Testing with %d processes.", numThreads)
	if testing.Short() {
		numRules = 100
	} else {
		numRules = 1000
	...
	experiment := mustNewExperiment(experimentDesc)
	wantAssessment, err := AssessRules(rules, experiment)
	if err != nil {
		t.Errorf("AssessRules(%v, %v) - err: %v", rules, experiment, err)
	}
	gotAssessment, err := AssessRulesMP(numThreads, rules, experiment)
	if err != nil {
		t.Errorf("AssessRulesMp(%d, %v, %v) - err: %v",
			numThreads, rules, experiment, err)
	}
	...
	if !assessmentsMatch {
		t.Errorf("AssessRulesMP output doesn't match AssessRules output\nassessments don't match\n - got: %v\n - wantRuleAssessments: %v, wantNumRecords: %d, wantIsSorted: %t, wantIsRefined: %t\n",
			gotAssessment, wantAssessment.RuleAssessments,
			wantAssessment.NumRecords, false, false)
	}
}

func TestAssessRulesMP_errors(t *testing.T) {
	cases := []struct {
		rules       []rule.Rule
		aggregators []*experiment.AggregatorDesc
		goals       []string
	...
	for _, c := range cases {
		...
		experiment := mustNewExperiment(experimentDesc)
		numThreads := 4
		_, err := AssessRulesMP(numThreads, c.rules, experiment)
		if err == nil || err.Error() != c.wantErr.Error() {
...
func BenchmarkAssessRulesMP(b *testing.B) {
	b.StopTimer()
	var numRules int64 = 3000
	numThreads := runtime.NumCPU()
	if numThreads < 2 {
		b.Skip("This test isn't implemented on single cpu systems.")
	}
	b.Logf("Testing with %d processes.", numThreads)
	rules := make([]rule.Rule, numRules)
	for i := int64(0); i < numRules; i++ {
		if i%2 == 0 {
			rules[i] = rule.NewGEFVI("age", i%50)
	...
	b.StartTimer()
	for n := 0; n < b.N; n++ {
		_, err := AssessRulesMP(numThreads, rules, experiment)
		if err != nil {
			b.Errorf("AssessRulesMP(%d, %v, %v) - err: %v",
				numThreads, rules, experiment, err)
		}
	}
}
